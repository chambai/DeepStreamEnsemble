...
from modules_detect_adapt import dse_adapt_ensemble_buffer
...
class AdaptEnsemble:
    ...
    def set_activation_ensemble(self, x_train, act_train_data, y_train, dnn_block):
        ...
        if self.use_clustering:
            ...
            self.clusterer = dse_adapt_ensemble_buffer.AdaptationEnsemble(copy.deepcopy(dnn_block.ens_dict))
    ...
    def start_model_update_thread(self):
        self.delay = util.getParameter('AdaptationTimeDelay')
        util.thisLogger.logInfo('AdaptationTimeDelay of %ss' % (self.delay))
    ...
    def adaptation(self, has_drift, act_unseen_batch, xdata_unseen_batch, dnnPredict_batch, streaming_classifier_predictions, drift_result, true_classes):
        majority_result = drift_result
        ...
        if has_drift == 1:
            ...
            if self.use_clustering:
                self.clus_lock.acquire()
                self.clusterer.partial_fit(xdata_unseen_batch, act_unseen_batch, true_classes)
                self.clus_lock.release()

            self.data_lock.acquire()
            self.concatenate_buffer_data(act_unseen_batch, xdata_unseen_batch, true_classes)
            self.data_lock.release()
        ...
        for r, d in zip(majority_result, dnnPredict_batch):
        ...
        self.calc_applied_adaptation(drift_result)
        if self.is_adapting:
    ...
    def calc_applied_adaptation(self, drift_result):
        if self.is_adapted:
            self.applied_adaptation.extend([1])
            self.applied_adaptation.extend([-1 for i in range(len(drift_result) - 1)])
        else:
            self.applied_adaptation.extend([-1 for i in range(len(drift_result))])
        self.is_adapted = False
    ...
    def stopProcessing(self):
        self.run_update_thread = False
        self.adapt = False
        util.thisLogger.logInfo('NumDriftTriggers=%s' % (self.drift_trigger_count))
        util.thisLogger.logInfo('NumAdaptations=%s' % (len(self.adapt_win_numbers)))
