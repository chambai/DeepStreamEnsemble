...
def startDataInputStream_DataStream(dnnModel, maxClassValue, unseenData):
    ...
    while stream.has_more_samples():
        ...
        batchTrueSuper = [u.correctResult for u in unseenDataBatches[batchIndex - 1]]
        if util.getParameter('DataDiscrepancy') == 'CD':
            batchTrueLabels = [u.subCorrectResult for u in unseenDataBatches[batchIndex - 1]]
        else:
        ...
        if 'modules_adapt' in moduleName:
            ...
            batch_result = module.processUnseenStreamBatch(batchDataInstances, batchActivations, batchDnnPredicts, batchTrueLabels, trueDiscrep, batchTrueSuper=batchTrueSuper)
    ...
    if hasattr(module, 'logTestAccuracy'):
        ...
        if len(ndDataInstances) > 0:
            if unseenData[0].reducedInstance.shape[0] > 1:
                ndActivations = np.array([x.reducedInstance for x in unseenData if x.discrepancyName == 'ND'])
            else:
