...
from modules_adapt.adapt_ensemble import AdaptEnsemble
from modules_adapt.dnnblockbase import DnnBlock
...
def setupAnalysis(indnn, act_train_batch, y_train_batch, inData=[]):
    global data_dir, dnn_block, adapt_ens, adaptive_dnn, is_adapted
    is_adapted = False
...
def processUnseenStreamBatch(xdata_unseen_batch, act_unseen_batch, dnnPredict_batch, true_values, true_discrep, batchTrueSuper):
    ...
    sim_drift = util.getParameter('SimulateDriftDetection')
    if sim_drift:
        result = []
        for td in true_discrep:
            if td != 'ND':
                ...
                result.append('N')
        streaming_classifier_predictions = batchTrueSuper
    else:
        result, streaming_classifier_predictions = dnn_block.processUnseenStreamBatch(xdata_unseen_batch, act_unseen_batch, dnnPredict_batch, true_values, true_discrep, batchTrueSuper)
    ...
    # # todo: remove - sets drift to true based on true discrepancies - dev only to test how well the adaptation works
    win_drift_val = 0
    if 'CD' in true_discrep or 'CE' in true_discrep:
        win_drift_val = 1
    ...
    adaptive_dnn, majority_result, y_classifier_batch, adapt_discrepancy, adapt_class = \
        adapt_ens.adaptation(win_drift_val, act_unseen_batch, xdata_unseen_batch, dnnPredict_batch, true_values, streaming_classifier_predictions, result, batchTrueSuper)
...
def stopProcessing():
    global data_dir, dnn_block, adapt_ens, adaptive_dnn
    dnn_block.applied_adaptation = adapt_ens.applied_adaptation
